...
o<=t.length&&t[o-1].scrollIntoView({behavior:"smooth",block:"center"})})),window.addEventListener("resize",s)})),document.addEventListener("DOMContentLoaded",(function(){const t=document.querySelectorAll(".ticket__inner"),e=document.querySelector(".ticket__block-more");//!Можна вказати скільки скільки потрібно відображати за замовчуванням
document.querySelector(".ticket__box-cards");//!Загальний контейнер в якому містяться всі картки
let o=2;//! Початкова кількість показаних елементів
function s(){window.innerWidth<=820?//! Вказати розмір до якого не буде реалізації Показати Ще/Сховати 
e.style.display="block":e.style.display="none",window.innerWidth<=820?//!Вказати який підходить розмір для приховування 
t.forEach(((t,e)=>{t.style.display=e<o?" block":"none"})):t.forEach((t=>{t.style.display=" block";//! Відображається елемент якщо потрібний flex/grid. Вказати flex/grid
})),
//! ПЕревірка, чи всі елементи вже відображені. Відповідно змінюється текст кнопки 
o===t.length?e.textContent="Esconder":e.textContent="Mostre mais"}s(),//! Викликаємо перевірку вікна при завантаженні сторінки
